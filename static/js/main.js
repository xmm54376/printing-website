...
(function() {
  ...
  if (quoteForm) {
    quoteForm.addEventListener('submit', function(e) {
      ...
      submitBtn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite;"><circle cx="12" cy="12" r="10" stroke-dasharray="60" stroke-dashoffset="60" opacity="0.3"/><path d="M12 2a10 10 0 010 20"/></svg> 提交中...';
      ...
      const productTypeText = productTypeEl ? productTypeEl.closest('.type-option').querySelector('.type-label').textContent.trim() : '';
      ...
      const payload = {
        ...
        length: parseFloat((document.getElementById('size-length') || {}).value) || 0,
        width: parseFloat((document.getElementById('size-width') || {}).value) || 0,
        height: parseFloat((document.getElementById('size-height') || {}).value) || 0,
        quantity: parseInt(qtyInput ? qtyInput.value : '0'),
        material: (document.getElementById('material') || {}).value || '',
        craft: processChecked.join(', '),
